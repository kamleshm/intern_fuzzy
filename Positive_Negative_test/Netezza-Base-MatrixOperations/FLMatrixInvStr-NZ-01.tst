...
--
--	Test Unit Number:	FLMatrixInvStr-NZ-01
--
--	Name(s):		FLMatrixInvStr
--
-- 	Description:		Calculates the inverse of a square matrix
--
--	Applications:		 
--
-- 	Signature:		FLMatrixInvStr(Row_ID, Col_ID, Cell_Val)
--
...
---- Simulate X
	...
	SELECT  p.MatrixID,
			FLMatrixRow(p.Inverse) AS Row,
			FLMatrixCol(p.Inverse) AS Col,
			FLMatrixVal(p.Inverse) AS Inverse
	FROM    ( 
			SELECT  a.MatrixID,
					FLMatrixInvStr(a.Row_id, a.Col_id, a.Cell_Val) OVER (PARTITION BY 1) AS Inverse 
			FROM   	tblMatrixInvTest a
			) AS p; 
	    
    ---- Inv(X) * X should return identity matrix
    SELECT a.MatrixID,
           CASE WHEN SumVal = 0 THEN 'Matrix-FT-FLMatrixInvStr-NZ-01P1: PASSED' ELSE 'Matrix-FT-FLMatrixInvStr-NZ-01P1: FAILED' END AS Msg
...
---- Simulate X
    ...
    ---- Inv(X) * X should return identity matrix
    SELECT a.MatrixID,
           CASE WHEN SumVal = 0 THEN 'Matrix-FT-FLMatrixInvStr-NZ-01P2: PASSED' ELSE 'Matrix-FT-FLMatrixInvStr-NZ-01P2: FAILED' END AS Msg
    FROM (SELECT a.MatrixID,
...
---- Simulate X
    ...
    ---- Inv(X) * X should return identity matrix
    SELECT a.MatrixID,
           CASE WHEN SumVal = 0 THEN 'Matrix-FT-FLMatrixInvStr-NZ-01P3: PASSED' ELSE 'Matrix-FT-FLMatrixInvStr-NZ-01P3: FAILED' END AS Msg
    FROM (SELECT a.MatrixID,
...
---- Inv(X) * X should return identity matrix
SELECT a.MatrixID,
       CASE WHEN SumVal = 0 THEN 'Matrix-FT-FLMatrixInvStr-NZ-01N2.4: PASSED' ELSE 'Matrix-FT-FLMatrixInvStr-NZ-01N2.4: FAILED' END AS Msg
FROM (SELECT a.MatrixID,
             Sum(CASE WHEN FLAbs(NumVal) <= 1e-4 THEN 0 ELSE 1 END) AS SumVal
      FROM (SELECT a.MatrixID,
                    a.Row_ID,
...
SELECT a.MatrixID,
       CASE WHEN SumVal = 0 THEN 'Matrix-FT-FLMatrixInvStr-NZ-01N3: PASSED' ELSE 'Matrix-FT-FLMatrixInvStr-NZ-01N3: FAILED' END AS Msg
FROM (SELECT a.MatrixID,
...
---- Inv(X) * X should return identity matrix
SELECT a.MatrixID,
       CASE WHEN SumVal = 0 THEN 'Matrix-FT-FLMatrixInvStr-NZ-01N4: PASSED' ELSE 'Matrix-FT-FLMatrixInvStr-NZ-01N4: FAILED' END AS Msg
FROM (SELECT a.MatrixID,
             Sum(CASE WHEN FLAbs(NumVal) <= 1e-6 THEN 0 ELSE 1 END) AS SumVal
      FROM (SELECT a.MatrixID,
                    a.Row_ID,
...
---- Inv(X) * X should return identity matrix
SELECT a.MatrixID,
       CASE WHEN SumVal = 0 THEN 'Matrix-FT-FLMatrixInvStr-NZ-01N5: PASSED' ELSE 'Matrix-FT-FLMatrixInvStr-NZ-01N5: FAILED' END AS Msg
FROM (SELECT a.MatrixID,
              Sum(CASE WHEN FLAbs(NumVal) <= 1e-6 THEN 0 ELSE 1 END) AS SumVal
       FROM (SELECT a.MatrixID,
                     a.Row_ID,
...
-------- N7.1 RowID <= 0
    ...
    ---- Inv(X) * X should return identity matrix
    SELECT a.MatrixID,
           CASE WHEN SumVal = 0 THEN 'Matrix-FT-FLMatrixInvStr-NZ-01N7.1: PASSED' ELSE 'Matrix-FT-FLMatrixInvStr-NZ-01N7.1: FAILED' END AS Msg
    FROM (SELECT a.MatrixID,
...
-------- N7.2 ColID <= 0                 
    ...
    ---- Inv(X) * X should return identity matrix
    SELECT a.MatrixID,
           CASE WHEN SumVal = 0 THEN 'Matrix-FT-FLMatrixInvStr-NZ-01N7.2: PASSED' ELSE 'Matrix-FT-FLMatrixInvStr-NZ-01N7.2: FAILED' END AS Msg
    FROM (SELECT a.MatrixID,
                 Sum(CASE WHEN FLAbs(NumVal) <= 1e-9 THEN 0 ELSE 1 END) AS SumVal
          FROM (SELECT a.MatrixID,
                        a.Row_ID,
